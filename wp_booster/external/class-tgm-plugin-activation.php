...
if ( ! function_exists( 'tgmpa' ) ) {
	...
	function tgmpa( $plugins, $config = array() ) {
		...
		foreach ( $plugins as $plugin ) {
			$GLOBALS['tgmpa']->register( $plugin );
		}

		if ( $config ) {
			$GLOBALS['tgmpa']->config( $config );
		}
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function __construct() {
			$this->admin_page_base = $GLOBALS['tgmpa']->parent_slug;
		...
		protected function _gather_plugin_data() {
			...
			$GLOBALS['tgmpa']->admin_init();
			$GLOBALS['tgmpa']->thickbox();
			...
			foreach ( $GLOBALS['tgmpa']->plugins as $plugin ) {
				if ( is_plugin_active( $plugin['file_path'] ) || ( isset( $plugin['is_callable'] ) && is_callable( $plugin['is_callable'] ) ) ) {
		...
		protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
			...
			foreach ( $GLOBALS['tgmpa']->plugins as $plugin => $values ) {
				if ( $name == $values['name'] && isset( $values[ $data ] ) ) {
					return $values[ $data ];
				}
			}
		...
		public function column_plugin( $item ) {
			...
			if ( ! isset( $installed_plugins[ $item['file_path'] ] ) ) {
				$install_nonce_url = wp_nonce_url(
					add_query_arg(
						array(
							'page'          => urlencode( $GLOBALS['tgmpa']->menu ),
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() ) {
				...
				if ( is_wp_error( $api ) ) {
					wp_die( $GLOBALS['tgmpa']->strings['oops'] . var_dump( $api ) );
				}
				...
				$url   = esc_url_raw( add_query_arg( array( 'page' => $GLOBALS['tgmpa']->menu ), self_admin_url( $this->admin_page_base ) ) );
				$nonce = 'bulk-plugins';
				$names = $plugin_names;
...
if ( ! function_exists( 'tgmpa_load_bulk_installer' ) ) {
	function tgmpa_load_bulk_installer() {
		...
		if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && $GLOBALS['tgmpa']->menu === $_GET['page'] ) ) {
				...
				class TGM_Bulk_Installer extends WP_Upgrader {
					...
					public function run( $options ) {
						...
						if ( $GLOBALS['tgmpa']->is_automatic ) {
							...
							$GLOBALS['tgmpa']->populate_file_path();
			...
			if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
				...
				class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function bulk_footer() {
						...
						foreach ( $GLOBALS['tgmpa']->plugins as $plugin ) {
							if ( ! is_plugin_active( $plugin['file_path'] ) ) {
								echo '<p><a href="' . esc_url( add_query_arg( 'page', $GLOBALS['tgmpa']->menu, self_admin_url( $this->admin_page_base ) ) ) . '" target="_parent">' . $GLOBALS['tgmpa']->strings['return'] . '</a></p>';
								$complete[] = $plugin;
								break;
							}
							// Nothing to store.
						...
						if ( empty( $complete ) ) {
							echo '<p>' . sprintf( $GLOBALS['tgmpa']->strings['complete'], '<a href="' . esc_url( self_admin_url() ) . '">' . __( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ) . '</p>';
							echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
						}
